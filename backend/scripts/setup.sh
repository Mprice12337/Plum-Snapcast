...
# Stop host Avahi if running (when in host network mode)
if [ -f /var/run/avahi-daemon/pid ]; then
    echo "Stopping host Avahi daemon to avoid conflicts..."
    kill $(cat /var/run/avahi-daemon/pid) 2>/dev/null || true
    sleep 2
fi

# Clear any stale Avahi state
rm -rf /var/run/avahi-daemon/* 2>/dev/null || true
mkdir -p /var/run/avahi-daemon
chmod 755 /var/run/avahi-daemon

# Test if we can start our own Avahi
echo "Starting Avahi daemon test..."
/usr/sbin/avahi-daemon --check || true
