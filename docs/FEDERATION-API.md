...
| Deployment Size | Typical Response | Notes |
|----------------|------------------|-------|
| Small (1-5 clients) | 10-20KB | No issues |
| Medium (10-50 clients) | 50-100KB | Suitable for all clients |
| Large (100+ clients) | 200KB+ | May exceed embedded device buffers |
| Very Large (500+) | 1MB+ | Consider pagination |

**Factors Affecting Size**:
- Album art encoded as base64 (adds ~50KB per active stream)
- Client metadata (hostname, OS info)
- Stream properties (full metadata per stream)

**For Embedded Clients** (ESP32, ESPHome):
- Typical buffer limit: 256KB
- Responses exceeding limit will be truncated
- Partial responses may still be parsable if truncated after relevant data
- Consider filtering by group or stream if deploying to 100+ client environments

**Future Enhancement**:
- Pagination support for `Server.GetStatus`
- Optional metadata filtering (`?include=streams,groups&exclude=album_art`)
...
Sent when a group's stream assignment changes.
...
**Fields**:
- `params.id` (string): Group UUID that was modified
- `params.stream_id` (string): New stream assignment
  - If starts with `"none-"`, group is now inactive/silent
  - Otherwise, group is listening to this stream

**Client Actions**:
- Update UI to show new stream for this group
- If cross-server federation enabled, may trigger server switch
- If stream is `"none-*"`, consider group inactive

**Frequency**:
- Sent when user changes stream via UI
- Sent when federation routes client to different stream
- Sent when stream lifecycle removes stream (routes to none)
...
#### Group ID Discovery After Server Switch

When a client switches servers, it must re-discover its group assignment:

**Flow**:
1. Client sends stream switch request to federation API
2. Federation routes remote snapclient to target stream
3. Client clears cached group ID (stale on new server)
4. Client polls new server with `Server.GetStatus`
5. Finds itself in server's group list, learns new group UUID
6. Caches group UUID for filtering future notifications

**Timing**:
- First notification after switch may arrive before group ID learned
- Clients should handle both scenarios:
  - **Known group ID**: Filter notifications by `params.id == our_group_id`
  - **Unknown group ID**: Accept all notifications, learn from first match

**Code Example**:
```python
async def handle_stream_changed(self, group_id: str, stream_id: str):
    if self.our_group_id is None:
        # Learning phase: accept notification and cache group ID
        self.our_group_id = group_id
        logger.info(f"Learned group ID: {group_id}")
    elif group_id != self.our_group_id:
        # Different group, ignore
        return

    # Process stream change for our group
...
**Purpose**: Silent streams for inactive clients (endpoint lockout)

**Naming Convention**:
- Format: `none-{identifier}`
- Example: `none-plum-snapcast`
- Automatically created by Snapcast on startup
- One per server

**Properties**:
- Status: Always "idle"
- URI: `pipe:///dev/zero?name=none`
- Cannot be removed or controlled

**Usage in Federation**:
- System-managed, do not route clients to none streams explicitly
- Federation router automatically routes inactive clients to none
- When scanning for active streams, filter out any with "none-" prefix

**Detection**:
```python
if stream_id.startswith("none-"):
    # This is a silence stream, ignore it
    pass
```

### 10.6. Client Identification Patterns

Clients use different ID formats based on type:

#### Hardware Clients (Output Endpoints)
**Format**: MAC address `aa:bb:cc:dd:ee:ff`
- Pattern: `^([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}$`
- Case: Lowercase from Snapcast (case-insensitive comparison recommended)
- Examples: `dc:a6:32:01:02:03`, `b8:27:eb:04:05:06`
- **Purpose**: Identify hardware devices outputting to speakers

#### Browser Clients (Snapweb)
**Format**: UUID `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`
- Generated by browser
- Example: `4dcc4e3b-c699-a04b-7f0c-8260d23c43e1`
- **Purpose**: Web-based clients for monitoring/playback

#### Remote Snapclients (Federation)
**Format**: `remote-{source-server-id}`
- Example: `remote-server-192-168-1-100`
- **Meaning**: Snapclient FROM server-192-168-1-100 (not TO)
- **Purpose**: Cross-server audio routing

#### ESPHome/Embedded Clients
**Format**: Custom via `--hostID` flag
- Recommended patterns:
  - MAC address: `aa:bb:cc:dd:ee:ff` (treated as hardware)
  - Descriptive: `esphome-kitchen`, `esp32-bedroom`
- Must match Snapcast's `client.id` field
...
def is_hardware_client(client_id: str) -> bool:
    """Hardware clients output audio (apply endpoint lockout)"""
    mac_pattern = r'^([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}$'
    return (bool(re.match(mac_pattern, client_id)) or
            client_id.startswith("remote-"))

def is_browser_client(client_id: str) -> bool:
    """Browser clients for monitoring (no endpoint lockout)"""
    uuid_pattern = r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    return bool(re.match(uuid_pattern, client_id, re.IGNORECASE))
```
