...
16. **Audio I/O Configuration**: Audio devices configured via GUI (Settings → Audio). **Output**: Device selection persists in `settings.json`, snapclient reloads on restart. SNAPCLIENT_SOUNDCARD env var deprecated (auto-migrated). **Input**: Enable devices to create Snapcast streams. Each enabled device creates an ALSA input stream via `manage_input_streams.py` (uses JSON-RPC API). Custom stream names configurable. Default: all inputs disabled. Supports all ALSA devices: built-in (headphones/HDMI), USB, HATs, microphones.
